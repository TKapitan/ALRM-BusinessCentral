/// <summary>
/// Table ART Assignable Range Header (ID 74179001).
/// </summary>
table 74179001 "ART Assignable Range Header"
{
    Caption = 'Assignable Range Header';
    LookupPageId = "ART Assignable Range List";
    DrillDownPageId = "ART Assignable Range List";
    DataClassification = CustomerContent;

    fields
    {
        field(1; "Code"; Code[20])
        {
            Caption = 'Code';
            DataClassification = CustomerContent;
        }
        field(2; Description; Text[50])
        {
            Caption = 'Description';
            DataClassification = CustomerContent;
        }
        field(10; "Ranges per BC Instance"; Boolean)
        {
            Caption = 'Ranges per BC Instance';
            DataClassification = SystemMetadata;

            trigger OnValidate()
            var
                ARTExtensionHeader: Record "ART Extension Header";

                CanNotChangeDueToExistingRecErr: Label 'The field can not be changed due to the existing extensions linked to this assignable range.';
            begin
                ARTExtensionHeader.SetRange("Assignable Range Code", Rec.Code);
                if not ARTExtensionHeader.IsEmpty() then
                    Error(CanNotChangeDueToExistingRecErr);
            end;
        }
        field(11; Default; Boolean)
        {
            Caption = 'Default';
            DataClassification = SystemMetadata;

            trigger OnValidate()
            var
                AssignableRangeHeader: Record "ART Assignable Range Header";

                CannotHaveMoreDefaultRangesErr: Label 'There is already existing default assignable range.';
            begin
                if Rec.Default and not xRec.Default then begin
                    AssignableRangeHeader.SetFilter(Code, '<>%1', Rec.Code);
                    AssignableRangeHeader.SetRange(Default, true);
                    if not AssignableRangeHeader.IsEmpty() then
                        Error(CannotHaveMoreDefaultRangesErr);
                end;
            end;
        }
        field(12; "Fill Object ID Gaps"; Boolean)
        {
            Caption = 'Fill Object ID Gaps';
            DataClassification = SystemMetadata;

            trigger OnValidate()
            var
                AssignableRangeLine: Record "ART Assignable Range Line";
            begin
                AssignableRangeLine.SetRange("Assignable Range Code", Rec.Code);
                AssignableRangeLine.ModifyAll("Fill Object ID Gaps", Rec."Fill Object ID Gaps");
            end;
        }
        field(15; "Default Object Range From"; Integer)
        {
            Caption = 'Default Object Range From';
            MinValue = 0;
            DataClassification = SystemMetadata;

            trigger OnValidate()
            begin
                ValidateChangeToDefaultRanges(RangeType::From, xRec."Default Object Range From", Rec."Default Object Range From");
            end;
        }
        field(16; "Default Object Range To"; Integer)
        {
            Caption = 'Default Object Range To';
            MinValue = 0;
            DataClassification = SystemMetadata;

            trigger OnValidate()
            begin
                ValidateChangeToDefaultRanges(RangeType::"To", xRec."Default Object Range To", Rec."Default Object Range To");
            end;
        }
        field(17; "Field Range From"; Integer)
        {
            Caption = 'Field Range From';
            MinValue = 0;
            DataClassification = SystemMetadata;

            trigger OnValidate()
            begin
                // TODO validate change to default field range
                // ValidateChangeToDefaultRanges(RangeType::From, xRec."Default Object Range From", Rec."Default Object Range From");
            end;
        }
        field(18; "Field Range To"; Integer)
        {
            Caption = 'Field Range To';
            MinValue = 0;
            DataClassification = ToBeClassified;

            trigger OnValidate()
            begin
                // TODO validate change to default field range
                // ValidateChangeToDefaultRanges(RangeType::"To", xRec."Default Object Range To", Rec."Default Object Range To");
            end;
        }
        field(25; "Object Name Template"; Text[30])
        {
            Caption = 'Object Name Template';
            DataClassification = CustomerContent;

            trigger OnValidate()
            var
                ARTExtensionLine: Record "ART Extension Object";
                TempInt: Integer;

                DifferentFileNamesExistsErr: Label 'There are extension lines that are different from the specified template name.';
            begin
                if Rec."Object Name Template" = '' then
                    exit;

                ARTExtensionLine.SetRange("Assignable Range Code", Rec.Code);
                TempInt := ARTExtensionLine.Count();
                ARTExtensionLine.SetFilter("Object Name", Rec."Object Name Template");
                if TempInt <> ARTExtensionLine.Count() then
                    Error(DifferentFileNamesExistsErr);
            end;
        }
        field(100; "No. Series for Extensions"; Code[20])
        {
            Caption = 'No. Series for Extensions';
            DataClassification = SystemMetadata;
            TableRelation = "No. Series".Code;
        }
    }

    keys
    {
        key(PK; "Code")
        {
            Clustered = true;
        }
    }

    fieldgroups
    {
        fieldgroup(DropDown; "Code", Description) { }
    }

    trigger OnDelete()
    var
        ARTAssignableRangeLine: Record "ART Assignable Range Line";
        ARTExtensionHeader: Record "ART Extension Header";

        CannotDeleteLinkExistsErr: Label '%1 can not be deleted due to existing related records in %2.', Comment = '%1 - Caption of table in which the record can not be deleted, %2 - Caption of table where the link exists.';
    begin
        ARTExtensionHeader.SetRange("Assignable Range Code", Rec."Code");
        if not ARTExtensionHeader.IsEmpty() then
            Error(CannotDeleteLinkExistsErr, Rec.TableCaption(), ARTExtensionHeader.TableCaption());

        ARTAssignableRangeLine.SetRange("Assignable Range Code", Rec.Code);
        ARTAssignableRangeLine.DeleteAll();
    end;

    trigger OnRename()
    var
        CanNotBeRenamedErr: Label 'The record can not be renamed.';
    begin
        Error(CanNotBeRenamedErr);
    end;

    var
        RangeType: Option From,To;
        MissingParameterErr: Label 'When the range has %1 = Yes, the IDs must be assigned using procedure that specify business central instance ID and the value must not be empty. This is probably programming error.', Comment = '%1 - Ranges per BC Instance field caption';

    /// <summary> 
    /// Allows to get new unused ID for specified object type
    /// </summary>
    /// <param name="ForObjectType">Enum "ART Object Type", The object type for which we want the ID</param>
    /// <param name="ForBusinessCentralInstance">Code[20], Code of .</param>
    /// <returns>Return variable "Integer" - specifies ID which is the next in row and is still unused.</returns>
    procedure GetNewObjectID(ForObjectType: Enum "ART Object Type"; ForBusinessCentralInstance: Code[20]): Integer
    var
        ARTExtensionObject: Record "ART Extension Object";
        ARTAssignableRangeLine: Record "ART Assignable Range Line";

        LastUsedObjectID, NewObjectID, VeryFirstObjectID : Integer;
    begin
        if Rec."Ranges per BC Instance" and (ForBusinessCentralInstance = '') then
            Error(MissingParameterErr, Rec.FieldCaption("Ranges per BC Instance"));

        LastUsedObjectID := 0;
        ARTExtensionObject.SetCurrentKey("Object Type", "Object ID");
        ARTExtensionObject.SetRange("Object Type", ForObjectType);
        ARTExtensionObject.SetRange("Assignable Range Code", Rec."Code");
        if Rec."Ranges per BC Instance" then begin
            // Find extension lines that are installed on specific business central instance
            ARTExtensionObject.SetRange("Bus. Central Instance Filter", ForBusinessCentralInstance);
            ARTExtensionObject.SetRange("Bus. Central Instance Linked", true);
        end;

        if ARTExtensionObject.FindLast() then begin
            // Try to find gap in already assigned ranges
            if FindGapInAssignableRange(ARTExtensionObject, ForObjectType, NewObjectID) then
                exit(NewObjectID);

            // Get next ID after the last used one (if the ID is within allowed ranges)
            LastUsedObjectID := ARTExtensionObject."Object ID";
            NewObjectID := LastUsedObjectID + 1;
            if IsObjectIDFromRange(ForObjectType, NewObjectID) then
                exit(NewObjectID);
        end else begin
            // No ID in use yet, find the first one
            VeryFirstObjectID := GetVeryFirstObjectID(ForObjectType);
            if VeryFirstObjectID <> 0 then
                exit(VeryFirstObjectID);
        end;

        // Some IDs already in use but the first range is depleted, try to find another defined range
        ARTAssignablerangeLine.SetRange("Assignable Range Code", Rec."Code");
        ARTAssignablerangeLine.SetRange("Object Type", ForObjectType);
        ARTAssignablerangeLine.SetFilter("Object Range To", '>%1', LastUsedObjectID);
        ARTAssignablerangeLine.FindFirst();
        exit(ARTAssignablerangeLine."Object Range From");
    end;

    /// <summary> 
    /// Allows to get new unused ID for specified object type.
    /// </summary>
    /// <param name="ForObjectType">Enum "ART Object Type", The object type for which we want the ID</param>
    /// <returns>Return variable "Integer" - specifies ID which is the next in row and is still unused.</returns>
    [Obsolete('Replaced by GetNewObjectID(Enum "ART Object Type"; Code[20])', '2021-04')]
    procedure GetNewObjectID(ForObjectType: Enum "ART Object Type"): Integer
    begin
        exit(GetNewObjectID(ForObjectType, ''));
    end;

    /// <summary> 
    /// Specifies whether the combination of the object type and ID is from any range specified by this assignable range code.
    /// </summary>
    /// <param name="ForObjectType">Enum "ART Object Type", The object type for which we want to check, whether the ID is within the range.</param> 
    /// <param name="ID">Integer, specifies ID of the object which we want to check.</param>
    /// <returns>Return variable "Boolean" - specifies whether the object type/ID is within any range in this assignable range code.</returns>
    procedure IsObjectIDFromRange(ForObjectType: Enum "ART Object Type"; ID: Integer): Boolean
    var
        ARTAssignableRangeLine: Record "ART Assignable Range Line";
    begin
        if ShouldUseDefaultRanges(ForObjectType) then begin
            if (Rec."Default Object Range From" <= ID) and (Rec."Default Object Range To" >= ID) then
                exit(true);
            exit(false);
        end;

        ARTAssignablerangeLine.SetRange("Assignable Range Code", Rec."Code");
        ARTAssignablerangeLine.SetRange("Object Type", ForObjectType);
        ARTAssignablerangeLine.SetFilter("Object Range From", '<=%1', ID);
        ARTAssignablerangeLine.SetFilter("Object Range To", '>=%1', ID);
        if not ARTAssignableRangeLine.IsEmpty() then
            exit(true);
        exit(false);
    end;

    /// <summary> 
    /// Returns the very first object ID from the range within which the ObjectID in parameter was created.
    /// </summary>
    /// <param name="ARTObjectType">Enum "ART Object Type", The object type for which we want the ID</param>
    /// <param name="ObjectID">Integer, object ID for which we are looking the very first ID.</param>
    /// <returns>Return variable "Integer", specifies ID of the very first object within the same range.</returns>
    procedure GetVeryFirstObjectIDFromRangeBasedOnObjectID(ARTObjectType: Enum "ART Object Type"; ObjectID: Integer): Integer
    var
        ARTAssignableRangeLine: Record "ART Assignable Range Line";
    begin
        if ShouldUseDefaultRanges(ARTObjectType) then
            exit(Rec."Default Object Range From");

        ARTAssignableRangeLine.SetRange("Assignable Range Code", Rec.Code);
        ARTAssignableRangeLine.SetRange("Object Type", ARTObjectType);
        if ObjectID <> 0 then begin
            ARTAssignableRangeLine.SetFilter("Object Range From", '<=%1', ObjectID);
            ARTAssignableRangeLine.SetFilter("Object Range To", '>=%1', ObjectID);
        end;
        ARTAssignableRangeLine.FindFirst();
        exit(ARTAssignableRangeLine."Object Range From");
    end;

    /// <summary> 
    /// Returns the very last object ID from the range within which the ObjectID in parameter was created.
    /// </summary>
    /// <param name="ARTObjectType">Enum "ART Object Type", The object type for which we want the ID</param>
    /// <param name="ObjectID">Integer, object ID for which we are looking the very last ID.</param>
    /// <returns>Return variable "Integer", specifies ID of the very last object within the same range.</returns>
    procedure GetVeryLastObjectIDFromRangeBasedOnObjectID(ARTObjectType: Enum "ART Object Type"; ObjectID: Integer): Integer
    var
        ARTAssignableRangeLine: Record "ART Assignable Range Line";
    begin
        if ShouldUseDefaultRanges(ARTObjectType) then
            exit(Rec."Default Object Range To");

        ARTAssignableRangeLine.SetRange("Assignable Range Code", Rec.Code);
        ARTAssignableRangeLine.SetRange("Object Type", ARTObjectType);
        if ObjectID <> 0 then begin
            ARTAssignableRangeLine.SetFilter("Object Range From", '<=%1', ObjectID);
            ARTAssignableRangeLine.SetFilter("Object Range To", '>=%1', ObjectID);
        end;
        ARTAssignableRangeLine.FindFirst();
        exit(ARTAssignableRangeLine."Object Range To");
    end;

    /// <summary> 
    /// Allows to get the very first object ID for specified object type. The ID is returned without checking whether is in use or not.
    /// </summary>
    /// <param name="ARTObjectType">Enum "ART Object Type", The object type for which we want the ID</param>
    /// <returns>Return variable "Integer" - specifies ID of the very first object in the range.</returns>
    local procedure GetVeryFirstObjectID(ARTObjectType: Enum "ART Object Type"): Integer
    begin
        exit(GetVeryFirstObjectIDFromRangeBasedOnObjectID(ARTObjectType, 0));
    end;

    /// <summary> 
    /// Specifies whether the default range should be used for specified object type instead of specific range.
    /// </summary>
    /// <param name="ForObjectType">Enum "ART Object Type", The object type for which we want to find out whether the default range should be used.</param>
    /// <returns>Return variable "Boolean" - whether the default range should be used.</returns>
    local procedure ShouldUseDefaultRanges(ForObjectType: Enum "ART Object Type"): Boolean
    var
        ARTAssignableRangeLine: Record "ART Assignable Range Line";
    begin
        ARTAssignableRangeLine.SetRange("Assignable Range Code", Rec."Code");
        ARTAssignableRangeLine.SetRange("Object Type", ForObjectType);
        if ARTAssignableRangeLine.IsEmpty() then
            exit(true);
        exit(false);
    end;

    /// <summary> 
    /// Allows to get new unused field ID for specified object type
    /// </summary>
    /// <param name="ForObjectType">Enum "ART Object Type", The object type for which we want the field ID</param>
    /// <param name="ForBusinessCentralInstance">Code[20], Code of the business central instance on which the object is used.</param>
    /// <returns>Return variable "Integer" - specifies field ID which is the next in row and is still unused.</returns>
    procedure GetNewFieldID(ForObjectType: Enum "ART Object Type"; ForBusinessCentralInstance: Code[20]): Integer
    var
        ARTExtensionObjectLine: Record "ART Extension Object Line";

        NewFieldID: Integer;
        NoAvailableFieldIDsErr: Label 'There are no available field IDs for a new field.';
    begin
        if Rec."Ranges per BC Instance" and (ForBusinessCentralInstance = '') then
            Error(MissingParameterErr, Rec.FieldCaption("Ranges per BC Instance"));

        ARTExtensionObjectLine.SetCurrentKey("Object Type", "ID");
        ARTExtensionObjectLine.SetAscending("ID", true);
        ARTExtensionObjectLine.SetRange("Object Type", ForObjectType);
        ARTExtensionObjectLine.SetRange("Assignable Range Code", Rec."Code");
        if Rec."Ranges per BC Instance" then begin
            // Find extension object lines that are installed on specific business central instance
            ARTExtensionObjectLine.SetRange("Bus. Central Instance Filter", ForBusinessCentralInstance);
            ARTExtensionObjectLine.SetRange("Bus. Central Instance Linked", true);
        end;

        if ARTExtensionObjectLine.FindLast() then begin
            NewFieldID := ARTExtensionObjectLine.ID + 1;
            if (Rec."Field Range From" <= NewFieldID) and (Rec."Field Range To" >= NewFieldID) then
                exit(NewFieldID);
            Error(NoAvailableFieldIDsErr);
        end;

        Rec.TestField("Field Range From");
        exit(REc."Field Range From");
    end;

    /// <summary> 
    /// Allows to get new unused field ID for specified object type
    /// </summary>
    /// <param name="ForObjectType">Enum "ART Object Type", The object type for which we want the field ID</param>
    /// <returns>Return variable "Integer" - specifies field ID which is the next in row and is still unused.</returns>
    [Obsolete('Replaced by GetNewFieldID(Enum "ART Object Type"; Code[20])', '2021-04')]/// 
    procedure GetNewFieldID(ForObjectType: Enum "ART Object Type"): Integer
    begin
        exit(GetNewFieldID(ForObjectType, ''));
    end;

    /// <summary> 
    /// Validate changes to default ranges to verify, whether the old range is not in use
    /// </summary>
    /// <param name="RangeType">Option (From,To), specify type of the range we want to validate.</param>
    /// <param name="OldRange">Integer, specify old range (the one before the change).</param>
    /// <param name="NewRange">Integer, specify new range (the one after the change).</param>
    local procedure ValidateChangeToDefaultRanges(RangeType: Option From,"To"; OldRange: Integer; NewRange: Integer)
    var
        ARTExtensionObject: Record "ART Extension Object";

        OptionNames: List of [Text];
        OptionString: Text;
        ARTObjectType: Enum "ART Object Type";

        OldRangeIsInUseErr: Label 'The range can not be change as there are extension lines with IDs from the existing range.';
    begin
        ARTExtensionObject.SetRange("Assignable Range Code", Rec.Code);
        if not ARTExtensionObject.ShouldCheckChange(RangeType, OldRange, NewRange) then
            exit;
        ARTExtensionObject.SetFilterOnRangeChange(RangeType, OldRange, NewRange);
        if ARTExtensionObject.IsEmpty() then
            exit;

        OptionNames := ARTObjectType.Names();
        foreach OptionString in ARTObjectType.Names() do begin
            Evaluate(ARTObjectType, OptionString);
            if ShouldUseDefaultRanges(ARTObjectType) then begin
                ARTExtensionObject.SetRange("Object Type", ARTObjectType);
                if not ARTExtensionObject.IsEmpty() then
                    Error(OldRangeIsInUseErr);
            end;
        end;
    end;

    /// <summary> 
    /// Check whether the object ID is already in use based on setting of this range
    /// </summary>
    /// <param name="ARTObjectType">Enum "ART Object Type", The object type which we want to check</param>
    /// <param name="ObjectID">Integer, ID of the object to check.</param>
    /// <param name="ForBusinessCentralInstance">Code[20], Code of the business central instance on which the object is used.</param>/// 
    /// <returns>Return variable "Boolean", whether the object is in use or not.</returns>
    procedure IsObjectIDAlreadyInUse(ARTObjectType: Enum "ART Object Type"; ObjectID: Integer; ForBusinessCentralInstance: Code[20]): Boolean
    var
        ARTExtensionObject: Record "ART Extension Object";
    begin
        ARTExtensionObject.SetRange("Object Type", ARTObjectType);
        ARTExtensionObject.SetRange("Object ID", ObjectID);
        if Rec."Ranges per BC Instance" then begin
            if ForBusinessCentralInstance = '' then
                Error(MissingParameterErr, Rec.FieldCaption("Ranges per BC Instance"));

            ARTExtensionObject.SetRange("Bus. Central Instance Filter", ForBusinessCentralInstance);
            ARTExtensionObject.SetRange("Bus. Central Instance Linked", true);
        end;

        if not ARTExtensionObject.IsEmpty then
            exit(true);
        exit(false);
    end;

    /// <summary> 
    /// Check whether the object name is already in use based on setting of this range
    /// </summary>
    /// <param name="ARTObjectType">Enum "ART Object Type", The object type which we want to check</param>
    /// <param name="ObjectName">Text, Name of the object to check.</param>
    /// <param name="ForBusinessCentralInstance">Code[20], Code of the business central instance on which the object is used.</param>/// 
    /// <returns>Return variable "Boolean", whether the object is in use or not.</returns>
    procedure IsObjectNameAlreadyInUse(ARTObjectType: Enum "ART Object Type"; ObjectName: Text; ForBusinessCentralInstance: Code[20]): Boolean
    var
        ARTExtensionObject: Record "ART Extension Object";
    begin
        ARTExtensionObject.SetRange("Object Type", ARTObjectType);
        ARTExtensionObject.SetRange("Object Name", ObjectName);
        if Rec."Ranges per BC Instance" then begin
            if ForBusinessCentralInstance = '' then
                Error(MissingParameterErr, Rec.FieldCaption("Ranges per BC Instance"));

            ARTExtensionObject.SetRange("Bus. Central Instance Filter", ForBusinessCentralInstance);
            ARTExtensionObject.SetRange("Bus. Central Instance Linked", true);
        end;

        if not ARTExtensionObject.IsEmpty then
            exit(true);
        exit(false);
    end;

    local procedure FindGapInAssignableRange(var ARTExtensionObject: Record "ART Extension Object"; ForObjectType: Enum "ART Object Type"; var NewObjectID: Integer): Boolean
    var
        ARTAssignableRangeLine: Record "ART Assignable Range Line";
        TempARTAssignableRangeLine: Record "ART Assignable Range Line" temporary;

        IDDiff, PrevID : Integer;
    begin
        TempARTAssignableRangeLine.DeleteAll();
        if ShouldUseDefaultRanges(ForObjectType) then begin
            if not Rec."Fill Object ID Gaps" then
                exit(false);

            TempARTAssignableRangeLine."Object Range From" := Rec."Default Object Range From";
            TempARTAssignableRangeLine."Object Range To" := Rec."Default Object Range To";
            TempARTAssignableRangeLine.Insert();
        end else begin
            ARTAssignableRangeLine.SetRange("Assignable Range Code", Rec.Code);
            ARTAssignableRangeLine.SetRange("Object Type", ForObjectType);
            ARTAssignableRangeLine.SetRange("Fill Object ID Gaps", true);
            if ARTAssignableRangeLine.FindSet() then
                repeat
                    TempARTAssignableRangeLine."Object Range From" := ARTAssignableRangeLine."Object Range From";
                    TempARTAssignableRangeLine."Object Range To" := ARTAssignableRangeLine."Object Range To";
                    TempARTAssignableRangeLine.Insert();
                until ARTAssignableRangeLine.Next() < 1;
        end;

        if TempARTAssignableRangeLine.FindSet() then
            repeat
                PrevID := 0;
                IDDiff := TempARTAssignableRangeLine."Object Range To" - TempARTAssignableRangeLine."Object Range From" + 1;
                if IDDiff > ARTExtensionObject.Count() then
                    if ARTExtensionObject.FindSet() then
                        repeat
                            if PrevID = 0 then begin
                                NewObjectID := TempARTAssignableRangeLine."Object Range From";
                                if ARTExtensionObject."Object ID" <> NewObjectID then
                                    if IsObjectIDFromRange(ForObjectType, NewObjectID) then
                                        exit(true);
                            end else begin
                                NewObjectID := PrevID + 1;
                                if (NewObjectID < ARTExtensionObject."Object ID") and IsObjectIDFromRange(ForObjectType, NewObjectID) then
                                    exit(true);
                            end;
                            PrevID := ARTExtensionObject."Object ID";
                        until ARTExtensionObject.Next() < 1;
            until TempARTAssignableRangeLine.Next() < 1;
        Clear(NewObjectID);
        exit(false);
    end;
}
